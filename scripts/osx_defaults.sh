...
# Prompt for computer name (as done via System Preferences → Sharing)
read -p "Enter computer name: " computerName
sudo scutil --set ComputerName "$computerName"

defaultHostName="Kevin Yap's Computer"
read -p "Enter host name [$defaultHostName]: " hostName
...
systemsetup -settimezone "America/Vancouver" > /dev/null
...
# Expand the “General” and “Open with” File Info panes:
...
# Make Safari's search match substrings
...
# Use the following to re-enable Spotlight tray icon.
...
# Change indexing order and enables all file types
...
cp ~/.dotfiles/conf/sublprefs.json \
   ~/Library/Application\ Support/Sublime\ Text\ 3/Packages/User/Preferences.sublime-settings 2> /dev/null
